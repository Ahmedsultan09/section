...
export default async function InquiryPage({
  ...
  return <main className={`inquiry-page subpage design-${design}`} data-design={design}>
    <JsonLd data={[
    ...
    <DesignSwitcher locale={locale} current={design} path={`/${locale}/inquiry`} />
    <aside className="inquiry-intro"><p className="eyebrow">SECTION / {locale === "ar" ? "استفسار مشروع" : "Project inquiry"}</p><h1>{locale === "ar" ? "ابدأ بما تحتاجه." : "Start with what you need."}</h1><p>{locale === "ar" ? "اختيارات بسيطة، ثم اسمك ورقم هاتفك. وإذا لم تكن متأكداً بعد، نساعدك في تحديد الخطوة التالية." : "A few simple choices, then your name and phone. If you are not sure yet, we can help shape the next step."}</p><div><span>01</span>{locale === "ar" ? "لا قوائم طويلة أو بيانات غير ضرورية" : "No long forms or unnecessary details"}</div><div><span>02</span>{locale === "ar" ? "الملاحظات والملفات اختيارية" : "Notes and files are optional"}</div></aside><InquiryForm locale={locale} />
  </main>;
}
