...
var client_references_default = {
	"15c18cfaeeff": async () => {
		const m = await import("./assets/layout-segment-context-CEDQIqhO.js");
		return { get "LayoutSegmentProvider"() {
			return m["LayoutSegmentProvider"];
		} };
	},
	"1a2e3044d30c": async () => {
		const m = await import("./assets/InquiryForm-CXz2fshH.js");
		return { get "InquiryForm"() {
			return m["InquiryForm"];
		} };
	},
	"33357993033b": async () => {
		const m = await import("./assets/ManufacturingLine-Ddl0LZSe.js");
		return { get "ManufacturingLine"() {
			return m["ManufacturingLine"];
		} };
	},
	"5297feacba07": async () => {
		const m = await import("./assets/ShowroomProcess-CB6I-aPp.js");
		return { get "ShowroomProcess"() {
			return m["ShowroomProcess"];
		} };
	},
	"593f344dc510": async () => {
		const m = await import("./assets/error-boundary-_JnU4AT9.js");
		return {
			get "ErrorBoundary"() {
	...
	"726cfa7c07d1": async () => {
		const m = await import("./assets/ProjectExplorer-BlUL4LJR.js");
		return { get "ProjectExplorer"() {
			return m["ProjectExplorer"];
		} };
	},
	"7bbb11e5bc50": async () => {
		const m = await import("./assets/NocturneCategoryStack-BDvdpeor.js");
		return { get "NocturneCategoryStack"() {
			return m["NocturneCategoryStack"];
		} };
	},
	"8c0f216c4604": async () => {
	...
	"9fb86276be8f": async () => {
		const m = await import("./assets/image-BeTdbdzN.js");
		return { get "default"() {
			return m["default"];
		} };
	},
	"a23e0279e836": async () => {
		const m = await import("./assets/LocaleSwitch-BLXYoapm.js");
		return { get "LocaleSwitch"() {
			return m["LocaleSwitch"];
		} };
	},
	"b178cf627e26": async () => {
		const m = await import("./assets/AdaptiveWebGL--55uv3Wu.js");
		return { get "AdaptiveWebGL"() {
			return m["AdaptiveWebGL"];
		} };
	},
	"b1e8e7503465": async () => {
		const m = await import("./assets/LeadDetailsDialog-CDUHRIME.js");
		return { get "LeadDetailsDialog"() {
			return m["LeadDetailsDialog"];
		} };
	},
	"c2747888630f": async () => {
		const m = await import("./assets/link-CEUN_oET.js").then((n) => n.n);
		return { get "default"() {
			return m["default"];
		} };
	},
	"f18800975cb3": async () => {
		const m = await import("./assets/ProjectStoryRail-XXq5lPpi.js");
...
//#endregion
export { __commonJSMin as C, stripBasePath as S, __toESM as T, isDangerousScheme as _, getPrefetchedUrls as a, VINEXT_MOUNTED_SLOTS_HEADER as b, usePathname as c, createRscRequestUrl as d, app_ssr_entry_default as default, isHashOnlyBrowserUrlChange as f, withBasePath as g, toSameOriginAppPath as h, handleSsr, getMountedSlotsHeader as i, useRouter as l, toBrowserNavigationHref as m, getCurrentInterceptionContext as n, navigateClientSide as o, resolveRelativeHref as p, getLayoutSegmentContext as r, prefetchRscResponse as s, require_jsx_runtime as t, createRscRequestHeaders as u, AppElementsWire as v, __exportAll as w, hasBasePath as x, require_react as y };
