...
const labels = {
  en: {
    ...
    required: "Choose an option before continuing.", contactRequired: "Add your name, phone number and consent before sending.", submitError: "We could not send the inquiry. Your answers are still here—please try again.",
    briefLabel: "Tell us anything you want us to know", consent: "I agree that SECTION can use these details to review my inquiry and contact me about the project.",
  ...
  ar: {
    ...
    required: "اختر إجابة قبل المتابعة.", contactRequired: "أدخل الاسم ورقم الهاتف والموافقة قبل الإرسال.", submitError: "تعذر إرسال الاستفسار. إجاباتك ما زالت محفوظة، حاول مرة أخرى.",
    briefLabel: "اكتب أي شيء تريد أن نعرفه", consent: "أوافق على استخدام SECTION لهذه البيانات لمراجعة الاستفسار والتواصل معي بخصوص المشروع.",
...
export function InquiryForm({ locale }: FormProps) {
  ...
  const validate = () => {
    const valid = step === 0
      ...
      : step === 1
        ? values.serviceScope.length > 0
        : step === 2
          ? values.projectStage.length > 0
          : step === 4
  ...
  const toggle = (key: "selectedCapabilities" | "serviceScope" | "projectStage", value: string) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    data.append("projectStage", JSON.stringify(values.projectStage.length ? values.projectStage : ["not-sure-yet"]));
...
function BriefStep({ copy, values, update }: { copy: Copy; values: Values; update: <K extends keyof Values>(key: K, value: Values[K]) => void }) {
  return <div className="brief-step"><label className="field wide"><span>{copy.briefLabel}</span><textarea rows={6} value={values.brief} onChange={(event) => update("brief", event.target.value)} /></label></div>;
}
